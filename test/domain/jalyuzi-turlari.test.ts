...
const QATORLAR: readonly SlotQatori[] = [
  ...
  { tur: 'RULON', slot: 'Val', birlik: 'SM', formula: 'MAX(40, ENI - 2)', koeff: 1, yonalish: 'ENIGA' },
  { tur: 'RULON', slot: 'Pastki planka', birlik: 'SM', formula: 'MAX(20, ENI - 2)', koeff: 1, yonalish: 'ENIGA' },
  { tur: 'RULON', slot: 'Zanjir', birlik: 'SM', formula: "BO'YI * 2 + 30", koeff: 1, yonalish: 'ENIGA' },
  ...
  { tur: 'VERTIKAL', slot: 'Karniz', birlik: 'SM', formula: 'ENI + 2', koeff: 1, yonalish: 'ENIGA' },
  ...
  { tur: 'VERTIKAL', slot: 'Pastki zanjircha', birlik: 'SM', formula: 'ENI + 10', koeff: 1, yonalish: 'ENIGA' },
  { tur: 'VERTIKAL', slot: 'Zanjir', birlik: 'SM', formula: "BO'YI * 2 + 25", koeff: 1, yonalish: 'ENIGA' },
  { tur: 'VERTIKAL', slot: 'Kronshteyn', birlik: 'DONA', formula: 'MAX(2, CEIL(ENI / 150))', koeff: 1, yonalish: 'ENIGA' },

  { tur: 'GORIZONTAL', slot: 'Lamel', birlik: 'SM', formula: "CEIL(BO'YI / QADAM) * ENI", koeff: 1, yonalish: 'ENIGA' },
  { tur: 'GORIZONTAL', slot: 'Lamel (dona)', birlik: 'DONA', formula: "CEIL(BO'YI / QADAM)", koeff: 1, yonalish: 'ENIGA' },
  { tur: 'GORIZONTAL', slot: 'Karniz', birlik: 'SM', formula: 'ENI', koeff: 1, yonalish: 'ENIGA' },
  { tur: 'GORIZONTAL', slot: 'Ip-lesa', birlik: 'SM', formula: "CEIL(ENI / 60) * (BO'YI + 20)", koeff: 1, yonalish: 'ENIGA' },
  { tur: 'GORIZONTAL', slot: "Ko'taruvchi ip", birlik: 'SM', formula: "CEIL(ENI / 60) * (BO'YI * 2 + ENI)", koeff: 1, yonalish: 'ENIGA' },
  ...
  { tur: 'PLISSE', slot: 'Yon profil', birlik: 'SM', formula: "(BO'YI + 2) * 2", koeff: 1, yonalish: 'ENIGA' },
  { tur: 'PLISSE', slot: 'Ip', birlik: 'SM', formula: "CEIL(ENI / 50) * (BO'YI * 2 + 20)", koeff: 1, yonalish: 'ENIGA' },
  ...
  { tur: 'RIM', slot: 'Reyka', birlik: 'SM', formula: "CEIL(BO'YI / BURMA) * ENI", koeff: 1, yonalish: 'ENIGA' },
  { tur: 'RIM', slot: 'Halqa', birlik: 'DONA', formula: "CEIL(BO'YI / BURMA) * 3", koeff: 1, yonalish: 'ENIGA' },
  { tur: 'RIM', slot: "Ko'taruvchi ip", birlik: 'SM', formula: "(BO'YI * 2 + ENI) * 3", koeff: 1, yonalish: 'ENIGA' },
];
...
describe('chekka o\'lchamlarda yiqilmaydi', () => {
  it.each(OLCHAMLAR.map(([e, b, izoh]) => [`${String(e)}×${String(b)} — ${izoh}`, e, b] as const))(
    '%s',
    (_nom, eni, boyi) => {
      const asos = standartQiymatlar(sm(eni), sm(boyi), 1, PARAMETRLAR);
  ...
  it('DONA sarfi butun songa yaxlitlanadi — yarim kronshteyn bo\'lmaydi', () => {
    const asos = standartQiymatlar(sm(260), sm(220), 1, PARAMETRLAR);
...
describe('slotSarfi — koeffitsient qaysi birlikka tegadi', () => {
  const asos = standartQiymatlar(sm(200), sm(100), 1, PARAMETRLAR);

  it('KV_M — koeffitsientga ko\'paytiriladi', () => {
    // 200 × 100 = 20 000 kv.sm = 2 kv.m; × 1.5 = 3
    expect(slotSarfi('MAYDON', asos, 'KV_M', 1.5)).toBeCloseTo(3, 4);
  });

  it('SM — koeffitsient E\'TIBORSIZ qoladi', () => {
    expect(slotSarfi('ENI', asos, 'SM', 1.5)).toBe(200);
  });
