...
export default async function KochirishSahifasi({
  params,
}: {
  ...
  const jonataOladi =
    beruvchimiz && h.holat === 'SOROV' && ruxsatBormi(f, 'ombor.kochirish.jonat');
  const qabulQilaOladi =
    qabulQiluvchimiz && h.holat === 'YOLDA' && ruxsatBormi(f, 'ombor.kochirish.qabul');
  const bekorQilaOladi =
    (h.holat === 'SOROV' || h.holat === 'YOLDA') &&
    ruxsatBormi(f, 'ombor.kochirish.jonat');
    ...
    <div className="flex flex-col gap-6">
      ...
      <dl className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white px-4 py-3">
          <dt className="text-xs uppercase tracking-wide text-slate-500">Bo&apos;lak</dt>
        ...
        <div className="rounded-xl border border-slate-200 bg-white px-4 py-3">
          <dt className="text-xs uppercase tracking-wide text-slate-500">Izoh</dt>
          <dd className="mt-1 text-sm">{h.izoh ?? '—'}</dd>
        </div>
      </dl>

      {h.holat === 'BEKOR' && (
        <p className="rounded-lg bg-red-50 px-3 py-2.5 text-sm text-red-900 ring-1 ring-red-200">
              ...
              {h.qatorlar.map((q) => (
                <tr key={q.bolakId}>
                  ...
                  <td className="raqam px-4 py-2.5">
                    ...
                    {q.olchovIzoh !== null && (
                      <div className="text-xs text-slate-500">{q.olchovIzoh}</div>
                    )}
                  </td>
